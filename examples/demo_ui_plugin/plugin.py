...
class DemoUIPlugin(BasePlugin):
    ...
    async def on_plugin_loaded(self) -> None:
        ...
        from Neo_MoFox_Webui.Plugin.utils.plugin_ui.plugin_ui_types import (  # type: ignore
            PageMode,
            PageRegistration,
        )
...
<page xmlns="urn:neo-mofox:plugin-ui:v3.1">
  <definitions>
  ...
  <vbox gap="1.5rem">
    ...
    <card title="添加条目" variant="outlined">
      <vbox gap="0.75rem">
        <sys-input label="名称" placeholder="输入条目名称..." value="{username}" on-change="set: username={username}" />
        <hbox gap="0.5rem">
    ...
    <card title="组件画廊" variant="outlined">
      <grid columns="2" gap="1rem">
        ...
        <vbox gap="0.5rem">
          ...
          <sys-switch label="启用功能" value="true" />
        </vbox>
        <vbox gap="0.5rem">
          <sys-text variant="subtitle">滑块</sys-text>
          <sys-slider label="音量" value="60" min="0" max="100" />
        </vbox>
      </grid>
    </card>
