...
import { getXmlComponent } from './xml-component-registry'
...
function renderSingleElement(element: Element, context: XmlRenderContext): VNode | null {
  ...
  // dialog id 自动绑定：把 <dialog id="x"> 展开为
  //   :open="store.get('__dialog_x_open') === true"
  //   @update:open="store.set('__dialog_x_open', $event)"
  //   @close="store.set('__dialog_x_open', false)"
  // 使作者侧语义 <dialog id="x"> + open-dialog: x / close-dialog: x 指令保持不变
  if (tagName === 'dialog') {
    const dialogId = element.getAttribute('id')
    if (dialogId && store) {
      const varPath = `__dialog_${dialogId}_open`
      // 仅当作者未显式 bind:open 时自动注入
      if (props.open === undefined) {
        props.open = store.get(varPath) === true
      }
      if (!props.onUpdateOpen) {
        props.onUpdateOpen = (v: boolean) => {
          store.set(varPath, v)
        }
      }
      if (!props.onClose) {
        props.onClose = () => {
          store.set(varPath, false)
        }
      }
    }
  }
